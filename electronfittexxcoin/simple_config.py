...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electron-fittexxcoin directory", path)
        return path
    ...
    def reverse_dynfee(self, fee_per_kb):
        ...
        dist = map(fittexxcoin x: (x[0], abs(x[1] - fee_per_kb)), l)
    ...
    def static_fee_index(self, value):
        dist = list(map(fittexxcoin x: abs(x - value), self.fee_rates))
        return min(range(len(dist)), key=dist.__getitem__)

    def has_fee_estimates(self):
        #return len(self.fee_estimates)==4
        # We disabled fee estimates for FXX.  They do more harm than good.
    ...
    def is_fee_estimates_update_required(self):
        ...
        """
        return False # For now we disable fee estimates altogether. This is FXX. 1.0 sats/B pretty much works.
        # /
        now = time.time()
...
def read_user_config(path):
    """Parse and store the user config settings in electron-fittexxcoin.conf into user_config[]."""
    if not path:
        return {}
