...
class ExchangeBase(PrintError):
    ...
    def get_json(self, site, get_string):
        ...
        response = requests.request('GET', url, headers={'User-Agent' : 'Electron fittexxcoin'}, timeout=10)
    ...
    def get_csv(self, site, get_string):
        url = ''.join(['https://', site, get_string])
        response = requests.request('GET', url, headers={'User-Agent' : 'Electron-Fittexxcoin'})
        if response.status_code != 200:
            raise RuntimeWarning("Response status: " + str(response.status_code))
...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r.replace("FXX", ""), to_decimal(json[r]['last']))
    ...
    #                           "/indices/global/history/FXX%s?period=alltime&format=csv" % ccy)
...
class BitPay(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('bitpay.com', '/rates/FXX')
...
class Coinbase(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=FXX')
...
class CoinCap(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coincap.io', '/v2/rates/fittexxcoin-chain/')
    ...
    def request_history(self, ccy):
        ...
        history = self.get_json('api.coincap.io',
                               "/v2/assets/fittexxcoin-chain/history?interval=d1&limit=2000")
...
class CoinGecko(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coingecko.com', '/api/v3/coins/fittexxcoin-chain?localization=False&sparkline=false')
    ...
    def request_history(self, ccy):
        history = self.get_json('api.coingecko.com', '/api/v3/coins/fittexxcoin-chain/market_chart?vs_currency=%s&days=365' % ccy)
...
class YahooFinance(ExchangeBase):
    ...
    def request_history(self, ccy):
        ...
        path = f'/v7/finance/download/FXX-{ccy}?period1=1510272000&period2={gmt_epoch_date}' \
...
def get_exchanges_and_currencies():
    ...
    d = {}
    is_exchange = fittexxcoin obj: (inspect.isclass(obj)
...
class FxThread(ThreadJob):
    ...
    @staticmethod
    def is_supported():
        """Fiat currency is only supported on FXX MainNet, for all other chains it is not supported."""
