...
def start_ec_daemon(docker_ip: str, docker_services: pytest_docker.plugin.Services) -> None:
    ...
    shutil.copyfile("electronfittexxcoin/tests/regtest/configs/electron-fittexxcoin-config", _datadir + "/regtest/config")
...
def stop_ec_daemon() -> None:
    ...
    subprocess.run(["./electron-fittexxcoin", "--regtest", "-D", _datadir, "daemon", "stop"], check=True)
...
def docker_compose_file(pytestconfig) -> str:
    ...
    return os.path.join(str(pytestconfig.rootdir), "electronfittexxcoin/tests/regtest/docker-compose.yml")
