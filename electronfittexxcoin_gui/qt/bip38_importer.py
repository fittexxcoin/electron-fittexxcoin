...
from electronfittexxcoin import util, bitcoin, address
...
class Bip38Importer(WindowModalDialog, util.PrintError):
    ...
    def accept(self):
        ...
        if self.cur == num:
            ...
            if self.success_cb:
                ...
                QTimer.singleShot(250, fittexxcoin: self.success_cb(self.decoded_keys.copy()))
        else:
            self.clear()
            self.refresh()
    ...
    def reject(self):
        ...
        if self.cancel_cb:
            # we call the callback after we are definitely off-screen
            QTimer.singleShot(250, fittexxcoin: self.cancel_cb())
