...
from electronfittexxcoin import keystore, Wallet, WalletStorage
from electronfittexxcoin.network import Network
from electronfittexxcoin.util import UserCancelled, InvalidPassword, finalization_print_error, TimeoutException
from electronfittexxcoin.base_wizard import BaseWizard
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def __init__(self, config, app, plugins, storage):
        ...
        self.rejected.connect(fittexxcoin: self.loop.exit(0))
        self.back_button.clicked.connect(fittexxcoin: self.loop.exit(1))
        self.next_button.clicked.connect(fittexxcoin: self.loop.exit(2))
        ...
        self.set_icon(':icons/electron-fittexxcoin.svg')
        self.show()
        self.raise_()
    ...
    def derivation_path_dialog(self, run_next, title, message, default, test, warning='', seed='', scannable=False):
        ...
        if scannable:
            ...
            hbox.addWidget(but)
            vbox.addLayout(hbox)
            but.clicked.connect(fittexxcoin: on_derivation_scan(line, seed))
