...
from electronfittexxcoin import util, get_config
...
class Console(QtWidgets.QWidget):
    ...
    def show_completions(self, completions):
        ...
        completions = map(fittexxcoin x: x.split('.')[-1], completions)
        t = '\n' + ' '.join(completions)
        if len(t) > 500:
            t = t[:500] + '...'
        c.insertText(t)
    ...
    def runCommand(self):
        ...
        if command:
            ...
            if type(self.namespace.get(command)) == type(fittexxcoin:None):
