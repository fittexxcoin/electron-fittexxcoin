...
import electronfittexxcoin
from electronfittexxcoin.address import Address
from electronfittexxcoin.util import format_satoshis, set_verbosity
from electronfittexxcoin.bitcoin import COIN, TYPE_ADDRESS
from electronfittexxcoin import Wallet, WalletStorage
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            sys.exit("Wallet not found. try 'electron-fittexxcoin create'")
    ...
    def print_addresses(self):
        fmt = "%-45s  %-30s"
        messages = map(fittexxcoin addr: fmt % (addr, self.wallet.labels.get(addr,"")), self.wallet.get_addresses())
    ...
    def run_popup(self, title, items):
        return self.run_dialog(title, list(map(fittexxcoin x: {'type':'button','label':x}, items)), interval=1, y_pos = self.pos+3)
