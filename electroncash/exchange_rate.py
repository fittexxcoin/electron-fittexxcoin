...
class ExchangeBase(PrintError):
    ...
    def get_csv(self, site, get_string):
        url = ''.join(['https://', site, get_string])
        response = requests.request('GET', url, headers={'User-Agent' : 'Electron-lambda'})
        if response.status_code != 200:
            raise RuntimeWarning("Response status: " + str(response.status_code))
...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r.replace("LMC", ""), to_decimal(json[r]['last']))
    ...
    #                           "/indices/global/history/LMC%s?period=alltime&format=csv" % ccy)
...
class BitPay(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('bitpay.com', '/rates/LMC')
...
class Coinbase(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=LMC')
...
class CoinCap(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coincap.io', '/v2/rates/lambda-chain/')
    ...
    def request_history(self, ccy):
        ...
        history = self.get_json('api.coincap.io',
                               "/v2/assets/lambda-chain/history?interval=d1&limit=2000")
...
class CoinGecko(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coingecko.com', '/api/v3/coins/lambda-chain?localization=False&sparkline=false')
    ...
    def request_history(self, ccy):
        history = self.get_json('api.coingecko.com', '/api/v3/coins/lambda-chain/market_chart?vs_currency=%s&days=365' % ccy)
...
class YahooFinance(ExchangeBase):
    ...
    def request_history(self, ccy):
        ...
        path = f'/v7/finance/download/LMC-{ccy}?period1=1510272000&period2={gmt_epoch_date}' \
...
class FxThread(ThreadJob):
    ...
    @staticmethod
    def is_supported():
        """Fiat currency is only supported on LMC MainNet, for all other chains it is not supported."""
