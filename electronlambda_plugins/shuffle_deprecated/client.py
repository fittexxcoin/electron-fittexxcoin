...
from electronlambda.bitcoin import deserialize_privkey, regenerate_key, EC_KEY, generator_secp256k1, number_to_string
from electronlambda.address import Address
from electronlambda.util import PrintError, InvalidPassword
from electronlambda.network import Network
from electronlambda import networks
from electronlambda.wallet import dust_threshold
from electronlambda.simple_config import get_config
...
class BackgroundShufflingThread(threading.Thread, PrintError):
    ...
    scales = (
        100000000000, # 1000.0  LMC ➡
        10000000000,  # 100.0   LMC ➡
        1000000000,   # 10.0    LMC ➡
        100000000,    #  1.0    LMC ➡
        10000000,     #  0.1    LMC ➝
        1000000,      #  0.01   LMC ➟
    ...
    DEFAULT_UPPER_BOUND = 9999990000    # (default) 99.99 LMC limit to max shuffle coin
    DEFAULT_LOWER_BOUND = SCALE_0 + FEE # (default) 0.0001 LMC + FEE minimum coin. Note config can never make minimum go below this.
