...
from electronlambda.bitcoin import (
...
from electronlambda.transaction import Transaction, int_to_hex
from electronlambda.address import Address, AddressError
from electronlambda.wallet import dust_threshold
from electronlambda.util import profiler, PrintError
import ecdsa
...
class CoinUtils(PrintError):
    ...
    def verify_tx_signature(signature, transaction, verification_key, utxo):
        ...
        try:
            ...
            return False
        if (s << 1) > order:
            # high S values are rejected by LMC network
            return False
        try:
