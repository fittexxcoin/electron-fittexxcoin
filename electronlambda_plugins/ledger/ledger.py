...
from electronlambda.bitcoin import TYPE_ADDRESS, TYPE_SCRIPT, int_to_hex, var_int
from electronlambda.i18n import _
from electronlambda.plugins import BasePlugin
...
from electronlambda.util import print_error, is_verbose, bfh, bh2u, versiontuple
...
MSG_NEEDS_FW_UPDATE_CASHADDR = _('Firmware version (or "Lambda" app) too old for CashAddr support. ') + \
        ...
        try:
            ...
            if (e.sw == 0x6faa):
                raise Exception(_("{hw_device_name} is temporarily locked - please unplug and plug it in again."
                                  "\n\nIf this problem persists please exit and restart the Lambda "
        ...
        if not self.preflightDone:
            try:
                ...
                if (e.sw == 0x6d00 or e.sw == 0x6700):
                    raise BaseException(_('{} not in Lambda mode').format(self.device)) from e
                raise e
