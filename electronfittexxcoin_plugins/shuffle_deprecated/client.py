...
from electronfittexxcoin.bitcoin import deserialize_privkey, regenerate_key, EC_KEY, generator_secp256k1, number_to_string
from electronfittexxcoin.address import Address
from electronfittexxcoin.util import PrintError, InvalidPassword
from electronfittexxcoin.network import Network
from electronfittexxcoin import networks
from electronfittexxcoin.wallet import dust_threshold
from electronfittexxcoin.simple_config import get_config
...
from .comms import Channel, ChannelWithPrint, ChannelSendFittexxcoin, Comm, query_server_for_stats, verify_ssl_socket, BadServerPacketError
...
class BackgroundShufflingThread(threading.Thread, PrintError):
    ...
    scales = (
        100000000000, # 1000.0  FXX ➡
        10000000000,  # 100.0   FXX ➡
        1000000000,   # 10.0    FXX ➡
        100000000,    #  1.0    FXX ➡
        10000000,     #  0.1    FXX ➝
        1000000,      #  0.01   FXX ➟
    ...
    DEFAULT_UPPER_BOUND = 9999990000    # (default) 99.99 FXX limit to max shuffle coin
    DEFAULT_LOWER_BOUND = SCALE_0 + FEE # (default) 0.0001 FXX + FEE minimum coin. Note config can never make minimum go below this.
    ...
    def _make_protocol_thread(self, scale, coins, scale_lower_bound, scale_upper_bound):
        def get_coin_for_shuffling(scale, coins, scale_lower_bound, scale_upper_bound):
            ...
            unshuffled_coins_on_scale.sort(key=fittexxcoin x: (x['value'], -x['height']))  # sort by value, preferring older coins on tied value
        ...
        thr.logger = ChannelSendFittexxcoin(fittexxcoin msg: self.protocol_thread_callback(thr, msg))
        cls = type(self)
