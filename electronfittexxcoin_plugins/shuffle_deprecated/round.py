...
#Processing the Blame phases
    ...
    def process_blame(self):
        ...
        handler = {
            ...
            br('Liar') : fittexxcoin p, r: self.logchan.send("Ignoring Liar message phase: {} reason: {}".format(p, r))
        }.get(reason)
        if handler:
    ...
    def process_blame_equivocation_failure(self, phase, reason):
        ...
        if self.is_inbox_complete(phase):
            ...
            key_cheaters = list(filter(fittexxcoin key: len(keys_matrix[key]) > 1, keys_matrix))
            change_cheaters = list(filter(fittexxcoin key: len(changes_matrix[key]) > 1, changes_matrix))
...
# Miscellaneous functions
    def check_and_blame_insufficient_funds(self):
        ...
        def compute_shuffle_amount():
            ...
            self.log_message("adjusts shuffle amount to {} FXX".format(self.shuffle_amount / 1e8))
