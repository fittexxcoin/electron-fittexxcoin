...
from electronfittexxcoin.bitcoin import (
...
from electronfittexxcoin.transaction import Transaction, int_to_hex
from electronfittexxcoin.address import Address, AddressError
...
class CoinUtils(PrintError):
    ...
    def make_unsigned_transaction(self, amount, fee, all_inputs, outputs, changes):
        ...
        tx_inputs.sort(key=fittexxcoin x: x['prevout_hash']+str(x["tx_pos"]))
    ...
    def verify_tx_signature(signature, transaction, verification_key, utxo):
        ...
        try:
            ...
            return False
        if (s << 1) > order:
            # high S values are rejected by FXX network
            return False
        try:
