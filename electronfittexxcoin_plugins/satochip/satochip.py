...
from electronfittexxcoin.bitcoin import TYPE_ADDRESS, int_to_hex, var_int
from electronfittexxcoin.i18n import _
from electronfittexxcoin.plugins import BasePlugin, Device
from electronfittexxcoin.keystore import Hardware_KeyStore
from electronfittexxcoin.transaction import Transaction
from electronfittexxcoin.wallet import Standard_Wallet
from electronfittexxcoin.util import print_error, bfh, bh2u, versiontuple, PrintError, is_verbose
from electronfittexxcoin.bitcoin import hash_160, Hash
from electronfittexxcoin.bitcoin import serialize_xpub
from electronfittexxcoin_gui.qt.qrcodewidget import QRCodeWidget, QRDialog
        ...
        for i,txin in enumerate(tx.inputs()):
            ...
            for j, x_pubkey in enumerate(x_pubkeys):
                ...
                if x_pubkey in derivations:
                    ...
                    if not is_cashtoken:
                        (response, sw1, sw2, tx_hash, needs_2fa) = client.cc.card_parse_transaction(pre_tx, True) # use 'True' since FXX use BIP143 as in Segwit...
...
class SatochipPlugin(HW_PluginBase):
    ...
    def create_seed(self, wizard):
        ...
        f = fittexxcoin x: self.request_passphrase(wizard, seed, x)
        wizard.show_seed_dialog(run_next=f, seed_text=seed)

    def request_passphrase(self, wizard, seed, opt_passphrase):
        if opt_passphrase:
            f = fittexxcoin x: self.confirm_seed(wizard, seed, x)
            wizard.passphrase_dialog(run_next=f)
        else:
            wizard.run('confirm_seed', seed, '')

    def confirm_seed(self, wizard, seed, passphrase):
        f = fittexxcoin x: self.confirm_passphrase(wizard, seed, passphrase)
        wizard.confirm_seed_dialog(run_next=f, test=fittexxcoin x: x==seed)

    def confirm_passphrase(self, wizard, seed, passphrase):
        f = fittexxcoin x: self.derive_bip39_seed(seed, x) #f = fittexxcoin x: self.derive_electrum_seed(seed, x)
        if passphrase:
            ...
            wizard.line_dialog(run_next=f, title=title, message=message, default='', test=fittexxcoin x: x==passphrase)
        else:
            f('')
    ...
    def restore_from_seed(self, wizard):
        ...
        f= fittexxcoin seed, is_bip39, is_ext: self.on_restore_seed(wizard, seed, is_bip39, is_ext)
    ...
    def on_restore_seed(self, wizard, seed, is_bip39, is_ext):
        ...
        if wizard.seed_type == 'bip39':
            f = fittexxcoin passphrase: self.derive_bip39_seed(seed, passphrase)
            ...
            wizard.confirm_dialog('Warning', message, run_next=fittexxcoin x: None)
            f = fittexxcoin passphrase: self.derive_electrum_seed(seed, passphrase)
            wizard.passphrase_dialog(run_next=f) if is_ext else f('')
